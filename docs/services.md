...
We have:
- `services/config`: It holds the configuration of the Token-SDK.
- `services/identity`: This is the identity service.
  It is responsible for the management of wallets, the long-term identities (X509 and Idemix), and the relative stores.
  It is used to back part of the Token and Driver API.
- `services/ttx`: This is the `token transaction service`.
  It helps the developers in assembling token requests and ultimately transactions for the backend.
  This package is backend agnostic and relies on the `network service` for backend specific operations.
- `services/network`: This is `network service`.
  It offers a unified way to handle different type of networks or backends (e.g. Fabric, and so on...).
...
- `services/storage`: This is `storage service`.
  It contains the storage places needed by the Token-SDK.   
  It is used to back part of the Token and Driver API.
...
Here is pictorial representation of the network service:
...
Otherwise, the `Connect` function on the network instance `Network` on input the desired namespace (taken also from the TMS ID).
The implementation of this function is responsible to establish a `connection` to the backend to serve the goals of the Token-SDK.
...
- One stream is used to analyze transactions that update the public parameters.
  In more details, for each transaction in a block, the parser checks if the RW set contains a write whose key is the `setup key`.
  The setup key is set as `\x00seU+0000`. If such a key is found in a valid transaction, then the listener added upon calling `Connect` does the following:
    - It invokes the `Update` function of the TMS provider passing the TMS ID and the byte representation of the new public parameters.
      The function works like this, if a TMS instance with the passed ID does not exists, it gets created.
      If a TMS with that ID already exists, then
        - A new instance of TMS is created with the new public parameters,
        - If the previous step succeeds, then the `Done` function on the old TMS instance is invoked to release all the allocated resources.
    - If above step succeeds then the public parameters are appended in `PublicParameters` table.
- The other stream is dedicated to the finality of transactions. Services can add listeners to the `Network` instance to listen for the finality of given transactions.
  ...
  Upon notification of the finality of a transaction for which the listener was added, the listener does the following:
    - If the transaction's status is valid, then the token request's hash contained in the transaction is matched against the hash of the token requested stored in the database.
      If they match then the `Tokens` table is updating inserting the new tokens and marking the spent tokens as deleted.
...
Tables:
- `Requests`: It contains the list of `Token Requests` assembled so far. The initial status of a token request is `Pending`.
  When finalized by the backed, it gets status `Valid`. If it rejected by the backend, it gets status `Invalid`.
...
- `Movements`: Once a transaction is finalized by the backend and its status becomes `Valid`, the transaction gets expanded in its constituent `movements`.
...
- `Endorsements`: It contains the `endorsement acks` received for a given token requests.
...
The tables `Tokens` and `TokenOnwers` are populated when the backed signaled that a given transaction previously assembled is valid.
...
The datamodel for this DB is identical to that of the transactions' db.

### Identity Store (`identitydb`),  Wallet Store (`walletdb`), and Key Store (`keystore`)

As we have seen before, a Wallet acts like a digital identity vault, holding a `long-term identity` and any `credentials derived from it`.
...
- `IdentityDB`: It is used to store identity configuration, signer related information, audit information, and so on.
  ...
  The `IdentityConfigurations` contains the following fields:
    - ID: a unique label to identity this identity configuration
    - TYPE: it is either "Owner", "Issuer", "Auditor", or "Certifier"
...
Such an entry contains information about the public version of `long-term` identity (X509 Certificate or Idemix Credential).
Secrets are either stored in an external `KeyStore` or spread between the `IdentityConfigurations` (raw field) and the Token-SDK's `Key Store`.
...
Depending on the implementation, the generation of these secret can happen on the remote storage.

The `IdentityInfo` contains also identities of external parties.
Imagine the scenario where Alice transfers token to Bob, Bob sends Alice the identity Bob wants to use to receive the tokens and Alice stores this information in the IdentityInfo table.
In this case, there is no signer associated to this identity, Alice knows then that this is not one of her identities.
...
Here is a pictorial representation of the lifecycle of a token transaction with Fabric as backend.
...
- The `Network` instance from the network service.
  This means that the system is configured with a backend that can be localed using the TMS ID.
...
- A Transaction ID that can be used for both a Fabric transaction and a Token Request as anchor.
- `NewRequest` is invoked on the TMS that returns an instance of an empty token request.

When a transaction is created,
it adds a hock to the FSC view context in such a way that if an error occurs all the resource used by the transaction are released (e.g. locked tokens).

### Request Recipient Identities

To both issue and transfer tokens, the initiator of these operations need to know the recipient identity to assign to the newly created token.
For concreteness, let us image the following scenario: Alice wants to transfer to Bob a token whose type is `TOK` of quantity `1`.
...
These views define an interactive protocol at end of which Alice learns Bob's recipient identity.

The views assume already that Alice and Bob know each other and their FSC stack is configured accordingly to enable the opening of an FSC communication session.

Then, `RequestRecipientIdentityView` does the following:
- Prepare a `RecipientRequest` message containing the TMS ID and Wallet ID.
- Opens an FSC communication session to the Bob passing to the FSC stack Bob's identifier as registered in the Alice's FSC endpoint service.
- Send the marshalled version of `RecipientRequest`.
- Wait for a message to come back

By the virtue of the FSC view framework, Bob executes `RespondRequestRecipientIdentityView` that does the following:
- Receive on the default FSC session the `RecipientRequest`.
- Validates it by checking that a TMS with the passed ID exists and a wallet with the passed ID exists.
  ...
  By using the TMS's WalletManager, the view checks that an owner wallet with the request ID exists.
  If the TMS or the wallet does not exist, then an error is returned to the initiator.
- Invoke the function `GetRecipientData` on the owner wallet.
  The owner wallet then generate on the fly or pick it from a cache, if available, an instance of `driver.RecipientData`.
  ...
  Another row in the table `IdentitySigners` is added to signal that this identity belongs to this node that created it.
  Another row is added to the `KeyStore` table with the secret key bound to the `identity` (the Idemix pseudonym).
- Send `RecipientData` back on the default FSC session.
- Update the FSC endpoint service by binding `RecipientData.identity` to its own FSC node identity.
...
- Register it in the `WalletManager` that checks that the information contained in `RecipientData` are correct and compatible with the public parameters.
  The registration results in a row added in the `IdentityInfo` table.
...
These are enough to perform a redeem and other variants of the fundamental operations.
A transaction helps to assemble these operations. A transaction can contain multiple operations at same time.
...
When invoked the function does invoke the corresponding `Issue` function on the `TokenRequest` (Token API) that does the following:
...
- Retrieves an Issuer identity from the issuer wallet via a call to `GetIssuerIdentity` for the request type.
  Currently, the same identity is retuned no matter the type. The identity returned correspond to that in a row of the `IdentityInfo` table.
...
When invoked the function does invoke the corresponding `Transfer` function on the `TokenRequest` (Token API) that does the following:
- Validates the inputs. For instance the quantities must be <= `MaxTokenValue`.
- Obtain a new `Selector` anchored to the ID of the transaction from the TMS's `SelectorManager` (Driver API).
- Invoke the `Selector`'s `Select` function to choose enough token to cover the total amount to transfer from the selected owner wallet.
  ...
  When a token is picked to be used, then the selector tries to acquire a lock over it by inserting a row in the `TokenLocks` table.
  If the insert is successful, then the token is reserved for this selector. If not, the selector tries another token.
- If the selector does not manage to get enough tokens to cover the demand, then an error is returned and all the locked tokens are releaded. This way, they can be used by other selectors.
- Use the `TransferService` from the Driver API to generate a transaction action as per the Token Driver identified by the TMS's public parameters.
...
The transaction contains a `Redeem` function to hide this aspect to the developer.
...
- A signature from the endorsers/approvers of the namespace this transaction refers too. (Recall that TMSID contains a namespace field that matches the namespace in Fabric).
...
Let us describe them in more details. The view takes in input a transaction and does the following:
...
- For each deduplicated identity collected, does the following:
    ...
    - If the node does not own that identity, then the view asks for a `remote signature`.
      ...
      This view can be used to receive either the marshalled version of a transaction of a `SignatureRequest` message.
      In both cases, `ReceiveTransactionView` returns a `Transaction`.
      The remove party can inspect the transaction and use the `AcceptView` to respond with the expected signature.
...
- Collect the endorsement/approval from the Fabric Endorsers of the given namespace (in the TMS ID).
  This operation is delegated to `network service` that is charge of dealing with the Fabric process to obtain an endorsement.
  Given the TMS ID, the relative network service is located and the function `RequestApproval` is invoked.
  The result of this operation is an instance of the network struct `Envolope`. This instance is attached to the transaction.
...
- (distribution step) For each recipient, the owner of a token, the view opens a session to the node that owns that identity.
  ...
  Indeed, image that Alice sends token to Bob and Charlie at the same time. Bob doesn't need to know which tokens Alice is using nor that Charlie has also received something.

The `AcceptView` is invoked inside a responding view for a given business process.
It is used evey time a party receives tokens.
When invoking this view, it is expected to invoke first `ReceiveTransactionView` to receive either a transaction of a `SignatureRequest` message.
...
- Respond to all the requests of signatures as defined in the token request. Don't forget that the token request must be inspected before calling the AcceptView.
  This means that for each expected `SignatureRequest`, the view check that it matches one of the expected.
    - Then, via the TMS's `SigService`, it obtains a signer for the identity whose signature is requested.
    - Generate the signature and send it back via the default session.
...
The `OrderingView` performs this operation. It takes in input a transaction and does the following:
- Get the network from the `network service` using the transaction's TMSID.
...
This view takes input either a transaction or a transaction id and does the following:
