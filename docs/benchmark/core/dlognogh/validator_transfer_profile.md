...
### 2. Deserialization (0.14%)
**`(*TokenRequest).FromBytes`** - Converts raw bytes to TokenRequest structure
- **Time:** 42.26µs (0.14%)
- **Purpose:** Parse the binary token request format

### 3. Main Verification (99.75%)
**`(*Unknown).VerifyTokenRequest`** - Orchestrates all validation checks
- **Time:** 29.71ms (99.75%)
- **Delegates to three main validation paths:**

#### 3.1 Auditing Validation (0.66%)
**`(*Unknown).VerifyAuditing`** → **`AuditingSignaturesValidate`**
- **Time:** 195.53µs (0.66%)
- **Purpose:** Verify auditor signatures on the request
- **Key Operations:**
  - Deserialize auditor verifier (51.89µs)
  - Verify signature (137.03µs)

#### 3.2 Action Deserialization (0.26%)
**`(*ActionDeserializer).DeserializeActions`** → **`(*Action).Deserialize`**
- **Time:** 77.14µs (0.26%)
- **Purpose:** Parse transfer actions from the request

#### 3.3 Transfer Verification (98.82%) - **CRITICAL PATH**
**`(*Unknown).verifyTransfers`** → **`(*Unknown).VerifyTransfer`**
- **Time:** 29.43ms (98.82%)
- **Purpose:** Validate each transfer action
- **Three main sub-validations:**

##### 3.3.1 Signature Validation (19.51%)
**`TransferSignatureValidate`**
- **Time:** 5.81ms (19.51%)
- **Purpose:** Verify sender signatures using Idemix credentials
- **Key Operations:**
  - **Owner Verifier Deserialization (17.52%):** Called twice (once per input)
    - Deserialize Idemix identity (5.22ms)
    - Validate identity proof (5.13ms)
  - **Signature Verification (1.88%):** Called twice
    - Verify pseudonym signature (555.57µs)

##### 3.3.2 Zero-Knowledge Proof Validation (79.10%) - **BOTTLENECK**
**`TransferZKProofValidate`** → **`(*Verifier).Verify`**
- **Time:** 23.56ms (79.10%)
- **Purpose:** Verify zero-knowledge proofs for transfer correctness
- **Key Operations:**

**a) Proof Deserialization (1.71%)**
- Deserialize type and sum proof (80.23µs)
- Deserialize range correctness proof (398.65µs)
  - Includes IPA (Inner Product Argument) data (185.03µs)

**b) Type and Sum Verification (2.75%)**
- Verify token types and amounts match (818.38µs)

**c) Range Correctness Verification (74.52%) - MAIN BOTTLENECK**
- **`(*RangeCorrectnessVerifier).Verify`** → **`(*rangeVerifier).Verify`** (called twice)
  ...
  - **Main Operation:** IPA Verification (69.87%)
    - **`(*rangeVerifier).verifyIPA`** (called twice, 20.81ms)
      - **`(*ipaVerifier).Verify`** (called twice, 5.75ms)
        - Serialize generators (228.45µs)
        - Clone generators (121.40µs)
        - **Compute S-vector (7.22%)** - Most expensive operation
          - Batch inverse computation (912.45µs)

##### 3.3.3 HTLC Validation (0.17%)
...
1. **Zero-Knowledge Proof Validation:** 78.95% (23.30ms)
   - Range correctness: 74.35% (21.95ms)
   - IPA verification: 69.97% (20.65ms)
2. **Signature Validation:** 19.66% (5.80ms)
   - Identity deserialization: 17.66% (5.21ms)
   - Signature verification: 1.90% (561.38µs)
3. **Auditing:** 0.56% (164.83µs)
4. **Other:** 0.51% (150.52µs)
...
```
(*rangeVerifier).verifyIPA x2 [20.65ms, 69.97%]
  └── (*ipaVerifier).Verify x2 [5.50ms, 18.62%]
      ├── (G1Array).Bytes x12 [0.15ms, 0.51%]
      ├── MarshalStd x2 [0.03ms, 0.11%]
      ├── cloneGenerators x2 [0.13ms, 0.43%]
      └── computeSVector x2 [2.17ms, 7.33%]
```

**Children sum:** 5.50 + 0.15 + 0.03 + 0.13 + 2.17 = **7.98ms**
**Parent time:** 20.65ms
**Difference:** 20.65 - 7.98 = **12.67ms** (61% of parent's time)

**Where's the missing 12.67ms?**
...
# Focus on VerifyTransfer subtree, hide functions < 1%
./profile.sh BenchmarkValidatorTransfer -f VerifyTransfer -m 1.0
```
...
**Safety:** The profiler works in an isolated temporary directory (`/tmp/profiler-<PID>`). Your repository files are never modified.
...
- [Range Proof Verifier](../../../token/core/zkatdlog/nogh/v1/crypto/rp/)
