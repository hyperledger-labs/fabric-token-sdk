...
with PdfPages(pdf_path) as pdf:
    ...
    for test_name in test_names:
        ...
        pattern = re.compile(rf"{re.escape(test_name)}-([a-zA-Z0-9_]+)/(\d+)\s+tps")
        ...
        for col in df.columns:
            ...
            if match:
                ...
                tps = last_row[col]
                lat_p95_col = f"{test_name}-{curve}/{worker} lat-p95"
                lat_avg_col = f"{test_name}-{curve}/{worker} lat-avg"
                lat_std_col = f"{test_name}-{curve}/{worker} lat-std"
