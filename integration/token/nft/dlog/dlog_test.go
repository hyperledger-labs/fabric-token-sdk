...
var _ = Describe("EndToEnd", func() {
	...
	Describe("NFT with libp2p", func() {
		var ts = nft.NewTestSuiteLibP2P("fabric", StartPortDlog, "dlog")
	...
	Describe("NFT with websockets", func() {
		var ts = nft.NewTestSuiteWebsocket("fabric", StartPortDlog, "dlog", integration.NoReplication)
		AfterEach(ts.TearDown)
		BeforeEach(ts.Setup)
		It("succeeded", func() { nft.TestAll(ts.II) })
	})
	...
	Describe("NFT with websockets and replicas", func() {
		var ts = nft.NewTestSuiteWebsocket("fabric", StartPortDlog, "dlog", &integration.ReplicationOptions{
