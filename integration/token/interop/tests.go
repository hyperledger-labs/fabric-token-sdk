...
func TestFastExchange(network *integration.Infrastructure, sel *token2.ReplicaSelector) {
	...
	Eventually(CheckBalance).WithArguments(network, sel.Get("bob"), "", "EUR", 10, token.WithTMSID(alpha)).WithTimeout(1 * time.Minute).WithPolling(15 * time.Second).Should(Not(Panic()))

	CheckBalance(network, sel.Get("alice"), "", "USD", 10, token.WithTMSID(beta))
	Eventually(CheckBalance).WithArguments(network, sel.Get("bob"), "", "USD", 20, token.WithTMSID(beta)).WithTimeout(1 * time.Minute).WithPolling(15 * time.Second).Should(Not(Panic()))
}
