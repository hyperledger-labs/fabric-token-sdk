...
func (v *FastExchangeInitiatorView) Call(context view.Context) (interface{}, error) {
	...
	me1, recipient, err := ttx.ExchangeRecipientIdentities(context, "", v.Recipient, token.WithTMSID(v.TMSID1))
	assert.NoError(err, "failed getting recipient identity")
	me2, sender, err := ttx.ExchangeRecipientIdentities(context, "", v.Recipient, token.WithTMSID(v.TMSID2))
	...
	_, err = view2.RunCall(context, func(context view.Context) (interface{}, error) {
		...
		wallet := ttx.GetWallet(context, "", token.WithTMSID(v.TMSID1))
	...
	_, err = view2.AsResponder(context, session,
		func(context view.Context) (interface{}, error) {
			...
			_, err = context.RunView(ttx.NewAcceptView(tx.Transaction))
			assert.NoError(err, "failed to accept new tokens")

			_, err = context.RunView(ttx.NewFinalityView(tx.Transaction))
	...
	_, err = view2.RunCall(context, func(context view.Context) (interface{}, error) {
		wallet := ttx.GetWallet(context, "", token.WithTMSID(v.TMSID2))
		assert.NotNil(wallet, "wallet not found")
...
func (v *FastExchangeResponderView) Call(context view.Context) (interface{}, error) {
	...
	// 1. Exchange recipient identities
	// 2. Agree on the terms
	me1, sender, err := ttx.RespondExchangeRecipientIdentities(context)
	assert.NoError(err, "failed to respond to identity request")

	me2, recipient, err := ttx.RespondExchangeRecipientIdentities(context)
	...
	_, err = view2.AsInitiatorCall(context, v, func(context view.Context) (interface{}, error) {
		...
		wallet := ttx.GetWallet(context, "", token.WithTMSID(terms.TMSID2))
		assert.NotNil(wallet, "wallet not found")
	...
	_, err = view2.AsInitiatorCall(context, v, func(context view.Context) (interface{}, error) {
		...
		// claim initiator's script
		wallet := ttx.GetWallet(context, "", token.WithTMSID(terms.TMSID1))
		assert.NotNil(wallet, "wallet not found")
