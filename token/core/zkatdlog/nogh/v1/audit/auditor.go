...
func (a *Auditor) Endorse(ctx context.Context, tokenRequest *driver.TokenRequest, txID string) ([]byte, error) {
	if tokenRequest == nil {
	...
	// Sign
	a.Logger.DebugfContext(ctx, "Endorse [%s][%s]", hash.Hashable(bytes).String(), txID)
	if a.Signer == nil {
