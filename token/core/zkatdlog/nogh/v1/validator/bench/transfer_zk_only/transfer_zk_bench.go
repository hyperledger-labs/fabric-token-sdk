...
func NewTokenTransferVerifyParamsSlice(TestRootPath string) []*trandferZKParams {
	if TestRootPath == "" {
	...
	ret := make([]*trandferZKParams, len(outPaths))
	for i, outPath := range outPaths {
		ret[i] = newTransferZKParams(filepath.Join(TestRootPath, outPath.Name()), curveID, numInputs, numOutputs, ppRaw)
	}

	return ret
}

func newTransferZKParams(outputPath string,
	curveID string,
	numInputs int,
	numOutputs int,
	ppRaw []byte,
...
type TransferZKView struct {
	params trandferZKParams
	proof  *ProofData
}
...
func (q *TransferZKView) Call(viewCtx view.Context) (interface{}, error) {
	if q.proof == nil {
...
type TransferZKViewFactory struct{}

// NewView builds a verification view.
// Wire proof embedded in the JSON params (remote/gRPC path)
func (c *TransferZKViewFactory) NewView(in []byte) (view.View, error) {
	f := &TransferZKView{}
