...
func (d *Driver) NewTokenService(tmsID driver.TMSID, publicParams []byte) (driver.TokenManagerService, error) {
	...
	authorization := common.NewAuthorizationMultiplexer(
		common.NewTMSAuthorization(ctx, logger, ppm.PublicParams(ctx), ws),
	...
	tokensUpgradeService, err := upgrade.NewService(logger, ppm.PublicParams(ctx).QuantityPrecision, deserializer, ip)
	if err != nil {
