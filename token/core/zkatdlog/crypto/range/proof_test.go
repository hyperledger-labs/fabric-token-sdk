...
func getRangeProver() *rp.Prover {
	...
	signer := getSigner(1)
	signatures[0], _ = signer.Sign([]*bn256.Zr{c.NewZrFromInt(0)})
	signatures[1], _ = signer.Sign([]*bn256.Zr{c.NewZrFromInt(1)})
	...
	prover := rp.NewProver([]*token.TokenDataWitness{tw}, []*bn256.G1{tok}, signatures, 2, pp, signer.PK, c.GenG1, signer.Q, c)

	return prover
}

func getSigner(length int) *pssign.Signer {
	s := pssign.NewSigner(nil, nil, nil, bn256.Curves[1])
	s.KeyGen(length)
	return s
}
