...
func prepareInputsForZKTransfer(pp *crypto.PublicParams) ([]*token.TokenDataWitness, []*token.TokenDataWitness, []*math.G1, []*math.G1) {
...
func prepareInvalidInputsForZKTransfer(pp *crypto.PublicParams) ([]*token.TokenDataWitness, []*token.TokenDataWitness, []*math.G1, []*math.G1) {
	c := math.Curves[pp.Curve]
	rand, err := c.Rand()
	Expect(err).NotTo(HaveOccurred())
