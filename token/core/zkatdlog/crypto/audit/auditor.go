...
func InspectTokenOwner(des Deserializer, token *AuditableToken, index int) error {
	...
	ro, err := identity.UnmarshallTypedIdentity(token.Token.Owner)
	if err != nil {
...
func inspectTokenOwnerOfScript(des Deserializer, token *AuditableToken, index int) error {
	owner, err := identity.UnmarshallTypedIdentity(token.Token.Owner)
	if err != nil {
	...
	ro, err := identity.UnmarshallTypedIdentity(scriptSender)
	if err != nil {
	...
	ro, err = identity.UnmarshallTypedIdentity(scriptRecipient)
	if err != nil {
