...
func TransferPledgeValidate(ctx *Context) error {
	ctx.Logger.Debug("pledge validation starts")
	for _, in := range ctx.InputTokens {
		id, err := identity.UnmarshalTypedIdentity(in.Owner.Raw)
		if err != nil {
		...
		if id.Type == pledge.ScriptType {
			...
			out := ctx.TransferAction.GetOutputs()[0].(*Output)
			tok := out.Output
			sender, err := identity.UnmarshalTypedIdentity(ctx.InputTokens[0].Owner.Raw)
			if err != nil {
				return err
			}
