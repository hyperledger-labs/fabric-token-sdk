...
func (s *Service[T]) NewCertificationRequest(ids []*token.ID) ([]byte, error) {
	return nil, nil
}
...
// zkatdlog does not make use of the certification service
func (s *Service[T]) Certify(wallet driver.CertifierWallet, ids []*token.ID, tokens [][]byte, request []byte) ([][]byte, error) {
	return nil, nil
}
...
// zkatdlog does not make use of the certification service
func (s *Service[T]) VerifyCertifications(ids []*token.ID, certifications [][]byte) ([][]byte, error) {
	return nil, nil
}
