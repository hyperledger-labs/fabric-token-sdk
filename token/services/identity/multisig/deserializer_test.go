...
func TestRecipients_Success(t *testing.T) {
	...
	recipients, err := deserializer.Recipients(context.Background(), nil, Multisig, idRaw)
	require.NoError(t, err)
...
func TestRecipients_InvalidRaw(t *testing.T) {
	...
	recipients, err := deserializer.Recipients(context.Background(), id, Multisig, raw)
	require.Error(t, err)
...
func (m *mockVerifierDES) DeserializeVerifier(ctx context.Context, id token.Identity) (driver.Verifier, error) {
...
func (m *mockAuditInfoMatcher) GetAuditInfoMatcher(ctx context.Context, owner token.Identity, auditInfo []byte) (driver.Matcher, error) {
