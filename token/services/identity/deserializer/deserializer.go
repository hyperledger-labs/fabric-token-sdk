...
func (e *EIDRHDeserializer) AddDeserializer(ctx context.Context, typ string, d driver2.AuditInfoDeserializer) {
...
func (e *EIDRHDeserializer) GetEIDAndRH(ctx context.Context, identity driver.Identity, auditInfo []byte) (string, string, error) {
	ai, err := e.getAuditInfo(context.Background(), identity, auditInfo)
	if err != nil {
		return "", "", err
	}
