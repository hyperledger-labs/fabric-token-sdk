	...
	for _, party := range signers {
		...
		if w := c.tx.TokenService().WalletManager().OwnerWalletByIdentity(party); w != nil {
			...
			if signer, err := c.tx.TokenService().SigService().GetSigner(party); err == nil {
				logger.Debugf("Received sigma: %s. Overriding with locally-calculated value.", base64.StdEncoding.EncodeToString(sigma))
				sigma, _ = signer.Sign(signatureRequest.MessageToSign())
				logger.Infof("Result overriden by: %s", base64.StdEncoding.EncodeToString(sigma))
			}
