...
func (a *DB) SetStatus(txID string, status TxStatus, statusMessage string) error {
	return a.db.SetStatus(txID, status, statusMessage)
}

// GetStatus return the status of the given transaction id.
// It returns an error if no transaction with that id is found
func (a *DB) GetStatus(txID string) (TxStatus, string, error) {
	st, sm, err := a.db.GetStatus(txID)
	if err != nil {
		return Unknown, "", err
	}
	return st, sm, nil
...
func (t *TxStatusChangesListener) OnStatusChange(txID string, status int, statusMessage string) error {
