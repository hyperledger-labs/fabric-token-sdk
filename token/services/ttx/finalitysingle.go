...
func newSingleFinalityWithOpts(opts ...TxOption) *singleFinalityView {
	return &singleFinalityView{opts: opts, pollingTimeout: pollingTimeout}
}

type singleFinalityView struct {
...
func (f *singleFinalityView) Call(ctx view.Context) (interface{}, error) {
	// Compile options
...
func (f *singleFinalityView) call(ctx view.Context, txID string, tmsID token.TMSID, timeout time.Duration) (interface{}, error) {
...
func (f *singleFinalityView) dbFinality(c context.Context, txID string, finalityDB finalityDB, startCounter, iterations int) (int, error) {
