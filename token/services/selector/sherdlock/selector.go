...
func NewStubbornSelector(logger logging2.Logger, tokenDB tokenFetcher, lockDB tokenLocker, precision uint64, backoff time.Duration, retries int) *stubbornSelector {
...
func NewSherdSelector(txID transaction.ID, fetcher tokenFetcher, lockDB LockDB, precision uint64, backoff time.Duration, maxRetriesAfterBackoff int) tokenSelectorUnlocker {
