...
func createManager(pgConnStr string, backoff time.Duration) (testutils.EnhancedManager, error) {
	...
	lockDB, err := common2.NewTokenLockDB(db, opts)
	if err != nil {
		return nil, err
	}
	tokenDB, err := common2.NewTokenDB(db, opts)
	if err != nil {
		return nil, err
	}
	...
	return testutils.NewEnhancedManager(NewManager(newMixedFetcher(tokenDB, tokenNotifier, newMetrics(&disabled.Provider{})), lockDB, testutils.TokenQuantityPrecision, backoff), tokenDB), nil
}
