...
// TestFetcherProvider_GetFetcher verifies errors when token store or notifier unavailable.
func TestFetcherProvider_GetFetcher(t *testing.T) {
	...
	t.Run("returns error when notifier not found", func(t *testing.T) {
		mockStoreManager := &mockStoreServiceManager{
			storeServiceByTMSIdFunc: func(tmsID token.TMSID) (*tokendb.StoreService, error) {
				return &tokendb.StoreService{}, nil
			},
		}
		mockNotifierManager := &mockNotifierManager{
			storeServiceByTMSIdFunc: func(tmsID token.TMSID) (*tokendb.Notifier, error) {
...
func (m *mockNotifierManager) StoreServiceByTMSId(tmsID token.TMSID) (*tokendb.Notifier, error) {
	if m.storeServiceByTMSIdFunc != nil {
		return m.storeServiceByTMSIdFunc(tmsID)
	}
