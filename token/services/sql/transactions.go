...
func (db *TransactionDB) StoreCertifications(certifications map[*token.ID][]byte) error {
	now := time.Now().UTC()
...
func (db *TransactionDB) ExistsCertification(tokenID *token.ID) bool {
	if tokenID == nil {
		return false
	}
	tokenIDStr := fmt.Sprintf("%s%d", tokenID.TxId, tokenID.Index)
...
func (db *TransactionDB) GetCertifications(ids []*token.ID, callback func(*token.ID, []byte) error) error {
	if len(ids) == 0 {
		// nothing to do here
		return nil
	}
