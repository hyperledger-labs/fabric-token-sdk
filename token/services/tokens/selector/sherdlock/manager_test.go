...
import (
	...
	testutils2 "github.com/hyperledger-labs/fabric-token-sdk/token/services/tokens/selector/testutils"
...
func TestSufficientTokensOneReplica(t *testing.T) {
	...
	defer terminate()
	testutils2.TestSufficientTokensOneReplica(t, replicas[0])
}
...
func TestSufficientTokensBigDenominationsOneReplica(t *testing.T) {
	...
	defer terminate()
	testutils2.TestSufficientTokensBigDenominationsOneReplica(t, replicas[0])
}

func TestSufficientTokensBigDenominationsManyReplicas(t *testing.T) {
	replicas, terminate := startManagers(t, 3, 2*time.Second, 10)
	defer terminate()
	testutils2.TestSufficientTokensBigDenominationsManyReplicas(t, replicas)
}

func TestInsufficientTokensOneReplica(t *testing.T) {
	replicas, terminate := startManagers(t, 1, NoBackoff, 5)
	defer terminate()
	testutils2.TestInsufficientTokensOneReplica(t, replicas[0])
}

func TestSufficientTokensManyReplicas(t *testing.T) {
	replicas, terminate := startManagers(t, 20, NoBackoff, 5)
	defer terminate()
	testutils2.TestSufficientTokensManyReplicas(t, replicas)
}

func TestInsufficientTokensManyReplicas(t *testing.T) {
	replicas, terminate := startManagers(t, 10, 5*time.Second, 5)
	defer terminate()
	testutils2.TestInsufficientTokensManyReplicas(t, replicas)
}

// Set up

func startManagers(t *testing.T, number int, backoff time.Duration, maxRetries int) ([]testutils2.EnhancedManager, func()) {
	terminate, pgConnStr := startContainer(t)
	replicas := make([]testutils2.EnhancedManager, number)
...
func createManager(pgConnStr string, backoff time.Duration, maxRetries int) (testutils2.EnhancedManager, error) {
	...
	manager := NewManager(fetcher, lockDB, testutils2.TokenQuantityPrecision, backoff, maxRetries, 0, 0)

	return testutils2.NewEnhancedManager(manager, tokenDB.(dbtest.TestTokenDB)), nil
}
