...
import (
	...
	inmemory2 "github.com/hyperledger-labs/fabric-token-sdk/token/services/tokens/selector/sherdlock/inmemory"
	"github.com/hyperledger-labs/fabric-token-sdk/token/services/tokens/selector/simple"
	"github.com/hyperledger-labs/fabric-token-sdk/token/services/tokens/selector/simple/inmemory"
	"github.com/hyperledger-labs/fabric-token-sdk/token/services/tokens/selector/testutils"
...
func NewSelector(qs *testutils.MockQueryService, walletIDByRawIdentity WalletIDByRawIdentityFunc, lock simple.Locker) (ExtendedSelector, CleanupFunction) {
	qf := func() simple.QueryService {
		return qs
	}

	s, _ := simple.NewManager(lock, qf, testutils.SelectorNumRetries, testutils.SelectorTimeout, false, testutils.TokenQuantityPrecision).NewSelector(testutils.TxID)
...
func NewSherdSelector(qs *testutils.MockQueryService, _ WalletIDByRawIdentityFunc, lock simple.Locker) (ExtendedSelector, CleanupFunction) {
	return &extendedSelector{
		Selector: sherdlock2.NewSherdSelector(testutils.TxID, sherdlock2.NewLazyFetcher(qs), inmemory2.NewLocker(lock), testutils.TokenQuantityPrecision, sherdlock2.NoBackoff, testutils.SelectorNumRetries),
		Lock:     nil,
	}, nil
}
...
type SelectorProviderFunction func(qs *testutils.MockQueryService, walletIDByRawIdentity WalletIDByRawIdentityFunc, lock simple.Locker) (ExtendedSelector, CleanupFunction)
type LockerProviderFunction func() simple.Locker
