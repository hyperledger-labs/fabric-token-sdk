...
func (cm *Manager) newOwner(tms *token.ManagementService) (*Owner, error) {
	...
	if err := net.SubscribeTxStatusChanges("", &AllStatusChangesListener{net, owner.db}); err != nil {
		return nil, errors.WithMessagef(err, "failed to subscribe event listener to network [%s:%s]", tms.ID().Network, tms.ID().Channel)
	}
	return owner, nil
}
