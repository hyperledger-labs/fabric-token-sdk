...
func (f RequestRecipientIdentityView) Call(context view.Context) (interface{}, error) {
	...
	if w := tms.WalletManager().OwnerWallet(f.Other); w != nil {
...
func (s *RespondRequestPledgeRecipientIdentityView) Call(context view.Context) (interface{}, error) {
	...
	ssp, err := state.GetServiceProvider(context)
	if err != nil {
	...
	if err != nil {
		return nil, errors.Wrapf(err, "failed getting recipient identity for wallet [%s]", wallet)
	}
	auditInfo, err := w.GetAuditInfo(recipientIdentity)
	if err != nil {
		return nil, errors.Wrapf(err, "failed getting audit info for recipient [%s], wallet [%s]", recipientIdentity, wallet)
	}
	metadata, err := w.GetTokenMetadata(recipientIdentity)
	if err != nil {
		return nil, errors.Wrapf(err, "failed getting token metadata for recipient [%s], wallet [%s]", recipientIdentity, wallet)
	}
