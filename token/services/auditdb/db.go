...
import (
	...
	"github.com/hyperledger-labs/fabric-token-sdk/token/services/auditdb/driver"
	driver2 "github.com/hyperledger-labs/fabric-token-sdk/token/services/ttxdb/driver"
...
func (db *DB) SetStatus(txID string, status TxStatus) error {
	...
	if err := db.db.SetStatus(txID, driver2.TxStatus(status)); err != nil {
		db.rollback(err)
	...
	for _, eID := range eIDs {
		for _, tokenType := range tokenTypes {
			...
			if err := db.db.AddMovement(&driver.MovementRecord{
				...
				Amount:       diff,
				TokenType:    tokenType,
				Status:       driver2.TxStatus(driver.Pending),
			}); err != nil {
		...
		for _, outEID := range outEIDs {
			for _, tokenType := range outTT {
				...
				if err := db.db.AddTransaction(&driver.TransactionRecord{
					...
					Amount:       received,
					Status:       driver2.TxStatus(driver.Pending),
					ActionType:   driver2.ActionType(tt),
					Timestamp:    timestamp,
				}); err != nil {
					if err1 := db.db.Discard(); err1 != nil {
						logger.Errorf("got error [%s]; discarding caused [%s]", err.Error(), err1.Error())
