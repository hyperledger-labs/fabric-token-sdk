...
func TransactionsTest(ctx context.Context, t *testing.T, cfgProvider cfgProvider) {
...
var tokenTransactionDBCases = []struct {
	Name string
	Fn   func(context.Context, *testing.T, driver.TokenTransactionStore)
}{
...
func TFailsIfRequestDoesNotExist(ctx context.Context, t *testing.T, db driver.TokenTransactionStore) {
...
func TStatus(ctx context.Context, t *testing.T, db driver.TokenTransactionStore) {
	tx := driver.TransactionRecord{
		TxID:         "tx1",
		ActionType:   driver.Transfer,
		SenderEID:    "bob",
	...
	txn := getTransactions(ctx, t, db, driver.QueryTransactionsParams{})[0]
	assert.Equal(t, driver.Pending, txn.Status, "transaction status should be pending")
	val := getValidationRecords(ctx, t, db, driver.QueryValidationRecordsParams{})[0]
...
func TStoresTimestamp(ctx context.Context, t *testing.T, db driver.TokenTransactionStore) {
	w, err := db.BeginAtomicWrite()
	assert.NoError(t, err)
	...
	txs := getTransactions(ctx, t, db, driver.QueryTransactionsParams{})
	assert.Len(t, txs, 1)
	assert.WithinDuration(t, now, txs[0].Timestamp, 3*time.Second)

	// Validation record (timestamp generated by code)
	vr := getValidationRecords(ctx, t, db, driver.QueryValidationRecordsParams{})
	assert.Len(t, vr, 1)
	assert.WithinDuration(t, now, vr[0].Timestamp, 3*time.Second)
}

func TMovements(ctx context.Context, t *testing.T, db driver.TokenTransactionStore) {
	w, err := db.BeginAtomicWrite()
	assert.NoError(t, err)
...
func TTransaction(ctx context.Context, t *testing.T, db driver.TokenTransactionStore) {
	...
	// get one
	one := getTransactions(ctx, t, db, driver.QueryTransactionsParams{IDs: []string{"tx10"}})
	assert.Len(t, one, 1)
	...
	records := getTransactions(ctx, t, db, driver.QueryTransactionsParams{Statuses: []driver.TxStatus{driver.Pending}})
	assert.Len(t, records, 19, "expect 19 pending")

	records = getTransactions(ctx, t, db, driver.QueryTransactionsParams{Statuses: []driver.TxStatus{driver.Confirmed}})
	...
	assert.NoError(t, w.AddTransaction(tr1))
	assert.NoError(t, w.Commit())
	noChange := getTransactions(ctx, t, db, driver.QueryTransactionsParams{ExcludeToSelf: true})
...
func assertTxEqual(ctx context.Context, t *testing.T, exp *driver.TransactionRecord, act *driver.TransactionRecord) {
	if act == nil {
...
func TTokenRequest(ctx context.Context, t *testing.T, db driver.TokenTransactionStore) {
	w, err := db.BeginAtomicWrite()
	assert.NoError(t, err)
...
func TAllowsSameTxID(ctx context.Context, t *testing.T, db driver.TokenTransactionStore) {
	...
	txs := getTransactions(ctx, t, db, driver.QueryTransactionsParams{})
	assert.Len(t, txs, 2)
	assertTxEqual(ctx, t, tr1, txs[0])
	assertTxEqual(ctx, t, tr2, txs[1])
}

func TRollback(ctx context.Context, t *testing.T, db driver.TokenTransactionStore) {
	w, err := db.BeginAtomicWrite()
	assert.NoError(t, err)
	...
	w.Rollback()
	assert.Len(t, getTransactions(ctx, t, db, driver.QueryTransactionsParams{}), 0)
	mvm, err := db.QueryMovements(driver.QueryMovementsParams{})
	assert.NoError(t, err)
	assert.Len(t, mvm, 0)
}

func TTransactionQueries(ctx context.Context, t *testing.T, db driver.TokenTransactionStore) {
	now := time.Now()
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			res := getTransactions(ctx, t, db, tc.params)
			assert.Len(t, res, tc.expectedLen, fmt.Sprintf("params: %v", tc.params))
		})
	}
}

func getTransactions(ctx context.Context, t *testing.T, db driver.TokenTransactionStore, params driver.QueryTransactionsParams) []*driver.TransactionRecord {
...
func TValidationRecordQueries(ctx context.Context, t *testing.T, db driver.TokenTransactionStore) {
	...
	all := getValidationRecords(ctx, t, db, driver.QueryValidationRecordsParams{})
	assert.Len(t, all, 4)
	...
	to := getValidationRecords(ctx, t, db, driver.QueryValidationRecordsParams{
		To: &beforeTx,
	})
	assert.Len(t, to, 0, "Expect no results if all records are created after 'To'")

	from := getValidationRecords(ctx, t, db, driver.QueryValidationRecordsParams{
		From: &beforeTx,
	})
	assert.Len(t, from, len(exp), "'From' before creation should include all records'")

	confirmed := getValidationRecords(ctx, t, db, driver.QueryValidationRecordsParams{
		Statuses: []driver.TxStatus{driver.Confirmed},
	})
	assert.Len(t, confirmed, 1)

	filtered := getValidationRecords(ctx, t, db, driver.QueryValidationRecordsParams{
...
func getValidationRecords(ctx context.Context, t *testing.T, db driver.TokenTransactionStore, params driver.QueryValidationRecordsParams) []*driver.ValidationRecord {
...
func TEndorserAcks(ctx context.Context, t *testing.T, db driver.TokenTransactionStore) {
	createTestTransaction(ctx, t, db, "1")
	wg := sync.WaitGroup{}
	n := 100
	wg.Add(n)
	for i := 0; i < n; i++ {
...
func createTestTransaction(ctx context.Context, t *testing.T, db driver.TokenTransactionStore, txID string) {
	w, err := db.BeginAtomicWrite()
	if err != nil {
