...
func TestTransactionsSqlite(t *testing.T) {
	...
	for _, c := range dbtest.Cases {
		db, err := initTransactionsDB("sqlite", fmt.Sprintf("file:%s?_pragma=busy_timeout(10000)", path.Join(tempDir, "db.sqlite")), c.Name, 10)
		if err != nil {
			t.Fatal(err)
		}
...
func TestTransactionsSqliteMemory(t *testing.T) {
	for _, c := range dbtest.Cases {
		db, err := initTransactionsDB("sqlite", "file:tmp?_pragma=busy_timeout(10000)&mode=memory&cache=shared", c.Name, 10)
		if err != nil {
			t.Fatal(err)
		}
		t.Run(c.Name, func(xt *testing.T) {
