...
func TestTokensSqlite(t *testing.T) {
	...
	for _, c := range TokensCases {
		db, err := initTokenDB("sqlite", fmt.Sprintf("file:%s?_pragma=busy_timeout(10000)&_pragma=foreign_keys(1)", path.Join(tempDir, "db.sqlite")), c.Name, 10)
		if err != nil {
			t.Fatal(err)
		}
...
func TestTokensSqliteMemory(t *testing.T) {
	for _, c := range TokensCases {
		db, err := initTokenDB("sqlite", "file:tmp?_pragma=busy_timeout(10000)&_pragma=foreign_keys(1)&mode=memory&cache=shared", c.Name, 10)
		if err != nil {
			t.Fatal(err)
		}
		t.Run(c.Name, func(xt *testing.T) {
