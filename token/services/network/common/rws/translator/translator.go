...
func (t *Translator) Write(ctx context.Context, action interface{}) error {
	...
	if err != nil {
		logger.ErrorfContext(ctx, "error committing transaction with txID '%s': %s", t.TxID, err)
		return err
	}
...
func (t *Translator) commitProcess(ctx context.Context, action interface{}) error {
	...
	if err != nil {
		logger.ErrorfContext(ctx, "error committing action with txID '%s': %s", t.TxID, err)
		return err
	}
