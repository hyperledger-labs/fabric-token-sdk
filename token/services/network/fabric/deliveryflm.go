...
func (p *deliveryBasedFLMProvider) NewManager(network, channel string) (FinalityListenerManager, error) {
	...
	logger.Debugf("Starting delivery service for [%s:%s]", network, channel)
...
func (m *deliveryBasedFLM) onBlock(ctx context.Context, block *common.Block) error {
	...
	logger.Infof("Invoked listeners for %d TxIDs: [%v]. Removing listeners...", invokedTxIDs)
	...
	logger.Debugf("Removed listeners for invoked TxIDs")
