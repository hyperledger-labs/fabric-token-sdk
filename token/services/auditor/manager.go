...
func (cm *Manager) newAuditor(w *token.AuditorWallet) (*Auditor, error) {
	...
	if err := net.SubscribeTxStatusChanges("", &AllStatusChangesListener{net, auditor.db}); err != nil {
		return nil, errors.WithMessagef(err, "failed to subscribe event listener to network [%s:%s]", w.TMS().Network(), w.TMS().Channel())
	}
	return auditor, nil
}
...
type AllStatusChangesListener struct {
	net *network.Network
	db  *ttxdb.DB
}

func (t *AllStatusChangesListener) OnStatusChange(txID string, status int) error {
