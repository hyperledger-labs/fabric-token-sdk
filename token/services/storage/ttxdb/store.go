...
// in that action.
type TransactionRecord = driver3.TransactionRecord
...
type MovementRecord = driver3.MovementRecord
...
// The transaction record contains the total amount of the token type that was transferred to/from that enrollment ID
// in that action.
type ValidationRecord = driver3.ValidationRecord

// TransactionIterator is an iterator over transaction records
type TransactionIterator struct {
	it driver3.TransactionIterator
}
...
type ValidationRecordsIterator struct {
	it driver3.ValidationRecordsIterator
}
...
type StoreService struct {
	*common.StatusSupport
	db    driver3.TokenTransactionStore
	cache Cache
}

func newStoreService(p driver3.TokenTransactionStore) (*StoreService, error) {
	return &StoreService{
...
type QueryTransactionsParams = driver3.QueryTransactionsParams

// QueryTokenRequestsParams defines the parameters for querying token requests
type QueryTokenRequestsParams = driver3.QueryTokenRequestsParams

// QueryValidationRecordsParams defines the parameters for querying movements
type QueryValidationRecordsParams = driver3.QueryValidationRecordsParams
...
func (d *StoreService) TokenRequests(ctx context.Context, params QueryTokenRequestsParams) (driver3.TokenRequestIterator, error) {
...
func (d *StoreService) SetStatus(ctx context.Context, txID string, status driver3.TxStatus, message string) error {
	logger.DebugfContext(ctx, "set status [%s][%s]...", txID, status)
	if err := d.db.SetStatus(ctx, txID, status, message); err != nil {
		return errors.Wrapf(err, "failed setting status [%s][%s]", txID, driver3.TxStatusMessage[status])
	}
	...
	logger.DebugfContext(ctx, "set status [%s][%s] done", txID, driver3.TxStatusMessage[status])
	return nil
}
	...
	for {
		...
		for _, outEID := range outEIDs {
			for _, tokenType := range outTT {
				...
				txs = append(txs, driver3.TransactionRecord{
...
func Movements(ctx context.Context, record *token.AuditRecord, created time.Time) (mv []MovementRecord, err error) {
	...
	for _, eID := range eIDs {
		for _, tokenType := range tokenTypes {
			...
			mv = append(mv, driver3.MovementRecord{
				TxID:         string(record.Anchor),
				EnrollmentID: eID,
				Amount:       diff,
				TokenType:    tokenType,
